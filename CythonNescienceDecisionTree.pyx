...
"""   
cdef int split(int n_values, double *values, int n_samples, double *X, int *y, int n_categories, int *counts):

    cdef int l_categories[1024]
    cdef int r_categories[1024]
    cdef int l_cat[1024]
    cdef int r_cat[1024]
    ...
    cdef int lc, rc, lt, rt
    
    cdef double ll, rl
    cdef double length, best_length
    cdef int best_value

    best_i  = -1
    best_length = n_samples * ( - log2(1 / <double>n_samples))

    # Initialize categories
    for i in range(n_categories):
        l_categories[i] = 0
        r_categories[i] = counts[i]
    ...
    for i in range(n_values):
        ...
        for j in range(n_samples):
            if X[j] == val:
                l_cat[y[j]] = l_cat[y[j]] + 1
                r_cat[y[j]] = r_cat[y[j]] - 1
            
        lt = 0
        rt = 0
        for j in range(n_categories):
            l_categories[j] = l_categories[j] + l_cat[j]
            r_categories[j] = r_categories[j] + r_cat[j]
            
            lt = lt + l_categories[j]
            rt = rt + r_categories[j]
        ...
        for j in range(n_categories):
            ...
            lc = l_categories[j]
            if lc != 0:
                ll = ll + lc * ( - log2(<double>lc / <double>lt))

            rc = r_categories[j]
            if rc != 0:
                rl = rl + rc * ( - log2(<double>rc / <double>rt))
...
cpdef int wrapper_split(values, X, y, n_categories, counts):

    cdef int  my_n_values = len(values)
    cdef np.ndarray[np.double_t, ndim=1, mode="c"] my_values
    my_values = np.ascontiguousarray(values, dtype=np.double)

    cdef int  my_n_samples = len(X)
    cdef np.ndarray[np.double_t, ndim=1, mode="c"] my_X
    my_X = np.ascontiguousarray(X, dtype=np.double)
    cdef np.ndarray[int, ndim=1, mode="c"] my_y
    my_y = np.ascontiguousarray(y, dtype=ctypes.c_int)

    cdef int  my_n_categories = n_categories
    cdef np.ndarray[int, ndim=1, mode="c"] my_counts
    my_counts = np.ascontiguousarray(counts, dtype=ctypes.c_int)

    best_i = split(my_n_values, &my_values[0], my_n_samples, &my_X[0], &my_y[0], my_n_categories, &my_counts[0])

    return best_i


class CythonNescienceDecisionTree(BaseEstimator):
    """
    ...
    """
    def _unique_count(self, x1, numeric1, x2=None, numeric2=None, both=False):
    ...
    """
    def _optimal_code_length(self, x1, numeric1, x2=None, numeric2=None, both=False):

        if both:
            count1, count2 = self._unique_count(x1=x1, numeric1=numeric1, x2=x2, numeric2=numeric2, both=True)
            ldm1 = np.sum(count1 * ( - np.log2(count1 / len(x1) )))
            ldm2 = np.sum(count2 * ( - np.log2(count2 / len(x1) )))
            return ldm1, ldm2
        else:
            count = self._unique_count(x1=x1, numeric1=numeric1, x2=x2, numeric2=numeric2, both=False)
    ...
    def _attributes_miscoding(self):
        ...
        for i in np.arange(self.X_.shape[1]):
            ...
            ldm_X, ldm_Xy = self._optimal_code_length(x1=self.X_[:,i], numeric1=self.X_isNumeric[i],
                                                      x2=self.y_, numeric2=self.isRegression, both=True)
    ...
    def _inaccuracy(self, y_hat):
        ...
        len_pred, len_joint   = self._optimal_code_length(x1=y_hat, numeric1=False,
                                                          x2=self.y_, numeric2=False, both=True)
    ...
    def _create_node(self, indices):
        ...
        results = Parallel(n_jobs=self.n_jobs)(delayed(self._create_node_helper)(i, self.X_isNumeric[i], X[:,i], y, counts, indices) for i in np.arange(self.X_.shape[1]))
    ...
    """   
    def _create_node_helper(self, attribute, isNumeric, X, y, counts, indices):
        ...
        best_i = wrapper_split(values, X, y, self.n_classes, counts)

        # Check if we have found a good split
        if best_i == -1:
            return None

        # Compute best inaccuracy
        lindices, rindices = self._split_data(attribute, values[best_i], indices, isNumeric)
        ...
        l_len = self._optimal_code_length(x1=self.y_[lindices], numeric1=False)
        r_len = self._optimal_code_length(x1=self.y_[rindices], numeric1=False)
...
class CythonNescienceDecisionTreeClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, cleanup=True, n_jobs=1, verbose=False):
        ...
        self.tree = CythonNescienceDecisionTree(verbose=verbose, mode="classification", n_jobs=n_jobs)
...
class CythonNescienceDecisionTreeRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, cleanup=True, n_jobs=1, verbose=False):
        ...
        self.tree = CythonNescienceDecisionTree(verbose=verbose, n_jobs=n_jobs, mode="regression")
